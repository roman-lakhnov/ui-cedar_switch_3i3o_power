...
import { useMatterState } from '@/hooks/relayState';
...
const Settings = () => {
  ...
  const {
    data: matterData,
    isLoading: matterIsLoading,
    isError: matterIsError,
  } = useMatterState();

  // const matterData = {
  //   commissioning_window_open: true,
  //   fabrics: 2,
  //   qr_code: 'MT:-24J0AFN00KA0648G00',
  //   pin_code: '34970112332',
  // };
  // const matterIsLoading = false;
  // const matterIsError = null;
  ...
  return (
    <div className='container'>
      ...
      <main className={styles.mainContent}>
        ...
        <section className={`card ${styles.matterControl}`}>
          <div className={styles.headingWrapper}>
            <div className={styles.dot}></div>
            <h2>Matter Control</h2>
          </div>

          {matterIsLoading && 'Loading...'}
          {matterIsError && 'No response'}
          {!matterIsLoading && !matterIsError && (
            <>
              {matterData?.commissioning_window_open ? (
								<>
                    <QRCodeSVG style={{ width: '100%', height: '100%' }} value={matterData.qr_code} />
                  <p className='pinPlaceholder'>PIN : {matterData.pin_code}</p>
                </>
              ) : (
                <div className={styles.inputGroup}>
                  <p>Pairing already happened</p>
                </div>
              )}
              <button className={`button ${styles.resetPairingButton}`}>
                Reset Pairing
              </button>
            </>
          )}
